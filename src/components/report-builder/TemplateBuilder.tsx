        ...
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <input
              ...
              className="w-full h-10 px-3 rounded-lg border border-neutral-200 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 focus-visible:border-accent-400"
            />
          </div>

          {/* Sectors */}
          <div>
            <label className="text-xs font-semibold text-neutral-700 mb-2 block">Sectors</label>
          ...
          <div>
            ...
            <div className="space-y-4">
              {questions.map((q, qi) => (
                ...
                >
                  ...
                  <div>
                    <label className="text-xs text-neutral-500 mb-1 block">Heading (optional)</label>
                    <input
                      value={q.heading || ''}
                      onChange={(e) => updateQuestion(q.id, { heading: e.target.value })}
                      placeholder="Section heading displayed in the report"
                      className="w-full h-9 px-3 rounded-lg border border-neutral-200 bg-white text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 focus-visible:border-accent-400"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full px-3 py-2 rounded-lg border border-neutral-200 bg-white text-sm resize-y focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 focus-visible:border-accent-400"
                    />
                  </div>

                  <div>
                    <label className="text-xs text-neutral-500 mb-1 block">Guidance / Instructions (optional)</label>
                    <textarea
                      value={q.guidance || ''}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label className="flex items-center gap-2 text-xs text-neutral-500 font-medium">
                    ...
                    <div className="space-y-2 mb-2">
                      {(q.sourcePriorities || []).map((group, groupIndex) => (
                        <div key={groupIndex} className="p-3 bg-white border border-neutral-200 rounded-lg space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-semibold text-neutral-600">Priority Sources {groupIndex + 1}</span>
                            <button 
                              onClick={() => {
                          ...
                          <div className="space-y-2">
                            {group.map((source, sourceIndex) => (
                              <div key={sourceIndex} className="flex items-center justify-between bg-neutral-50 px-3 py-2 rounded border border-neutral-100 text-xs text-neutral-700">
                                <span>{source}</span>
                                <button 
                                  onClick={() => {
                                    const newPriorities = [...(q.sourcePriorities || [])];
                          ...
                          <select
                            ...
                            className="w-full h-8 px-2 rounded border border-neutral-200 text-xs bg-white text-neutral-500 focus-visible:outline-none focus:ring-1 focus-visible:ring-accent-500"
                          >
                    ...
                    <button
                      ...
                      className="px-3 py-1 bg-green-600 hover:bg-green-700 text-white text-xs font-medium rounded transition-colors inline-flex items-center gap-1"
                    >
                      ...
                      {(['text', 'table', 'chart', 'sentiment'] as AnswerFormat[]).map(fmt => (
                        <label key={fmt} className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            className="rounded border-neutral-300 text-accent-600 focus-visible:ring-accent-500"
                          />
            ...
            <button
              onClick={addQuestion}
              className="mt-3 flex items-center gap-2 text-xs font-medium text-accent-600 hover:text-accent-700 transition-colors"
            >
